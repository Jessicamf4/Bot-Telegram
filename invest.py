...
def aposentadoria_receive(update: Update, context: CallbackContext) -> int:
    if category == "Aposentadoria":
        ...
        rendimentoTotal = ((float(text)*(1.04))**30)
        anos=[1, 30]
        renda=[text, rendimentoTotal]
        ...
        f"{facts_to_str(user_data)} você deseja investir {text} e deixá-lo guardado durante 30 anos! Ótimo! \n"
...
def received_information(update: Update, context: CallbackContext) -> int:
    ...
    elif category == "Trinta anos":
        calculo = float(text) * 1.6
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            CHOOSING: [
                ...
                MessageHandler(Filters.regex('^(Aposentadoria|Something else...)$'), custom_choice),
            ],
            TYPING_CHOICE: [
                MessageHandler(
            ...
            TYPING_REPLY: [
                MessageHandler(
                    ...
                    received_information, aposentadoria_receive,
                )
            ],
        },
